...
-- This one is deliberately an advocacy piece, not a balanced review: it makes
-- the case for watching the series while it airs. It therefore quotes no
-- critics at all rather than cropping the mixed notices (THR, Variety) down to
-- their favourable halves, which would misrepresent them.
